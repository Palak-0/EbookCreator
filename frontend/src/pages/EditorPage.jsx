...
const EditorPage = () => {
    ...
    const fetchBook = async () => {
      try {
        const response = await axiosInstance.get(
          API_PATHS.BOOKS.GET_BOOK_BY_ID.replace(":id", bookId),
        );
  ...
  const handleSaveChanges = async (bookToSave = book, showToast = true) => {
    ...
    try {
      await axiosInstance.put(
        API_PATHS.BOOKS.UPDATE_BOOK.replace(":id", bookId),
        bookToSave,
      );
      if (showToast) {
  ...
  const handleCoverImageUpload = async (e) => {
    ...
    try {
      const response = await axiosInstance.put(
        API_PATHS.BOOKS.UPDATE_COVER.replace(":id", bookId),
        formData,
        {
          headers: {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        API_PATHS.EXPORT.PDF.replace(":id", bookId),
        { responseType: "blob" },
      );
  ...
  const handleExportDoc = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        API_PATHS.EXPORT.DOC.replace(":id", bookId),
        { responseType: "blob" },
      );
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
